...
class Game:
    ...
    def switch_map(self, map_name, spawn, direction):
        tmx_data = pytmx.util_pygame.load_pygame(f"asset/{map_name}.tmx")
        map_data = pyscroll.data.TiledMapData(tmx_data)
        map_layer = pyscroll.orthographic.BufferedRenderer(
            map_data, self.screen.get_size()
        )
        ...
        enter_house = tmx_data.get_object_by_name(direction)
        self.enter_house = pygame.Rect(enter_house.x, enter_house.y, enter_house.width, enter_house.height)
    ...
    def handle_collisions(self):
        ...
        if self.player.rect.colliderect(self.enter_house):
            if self.map_name == "Desert":
                self.switch_map("House", "spawn_indoor", "exit")
                self.map_name = "House"
            else:
                self.switch_map("Desert", "spawn_outdoor", "enter")
                self.map_name = "Desert"
